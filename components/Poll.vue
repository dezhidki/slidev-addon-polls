<template>
  ...
  <!-- ===== AUDIENCE VIEW ===== -->
  <div v-else-if="!isPresenter" class="poll-audience-root">
    <div class="poll-header">
      <span class="poll-title">📊 {{ audienceQuestion }}</span>
      <div class="poll-meta">
        <span class="slide-num">Slide {{ slideIndex }}</span>
        <span class="poll-count">👥 {{ localAudience }}</span>
        <span class="poll-dot" :class="audienceConnected ? 'on' : 'off'" />
      </div>
    </div>

    <div v-if="!audienceConnected" class="poll-offline">🔄 Connecting to poll server…</div>

    <div v-if="audienceActive && audienceConnected" class="poll-body">
      <!-- Word cloud -->
      <div v-if="audienceActive.type === 'wordcloud'" class="wc-area">
        <div v-if="!audienceWcWords.length" class="empty">Waiting for responses…</div>
        <div class="cloud-wrap">
          <span
            v-for="(w, i) in audienceWcWords"
            :key="i"
            class="cloud-word"
            :style="{ fontSize: wcSz(w.count) + 'px', opacity: wcOp(w.count) }"
          >{{ w.word }}</span>
      ...
      <div v-else-if="audienceActive.options && audienceActive.options.length" class="options">
        <div
          v-for="(opt, i) in audienceActive.options"
          :key="i"
          class="opt-row"
          :class="{
            leading: audienceIsLead(i) && audienceTotalVotes > 0,
            correct: showReveal && audienceActive.correctAnswer === i,
          }"
        >
          <div class="opt-label">
            <span class="opt-text">{{ optText(opt) }}</span>
            <span v-if="showReveal && audienceActive.correctAnswer === i" class="correct-chip">✓</span>
          </div>
          <div class="opt-bar-wrap">
            <div
              class="opt-bar"
              :style="{ width: audiencePct(i) + '%' }"
              :class="{
                'bar-leading': audienceIsLead(i) && audienceTotalVotes > 0,
                'bar-correct': showReveal && audienceActive.correctAnswer === i,
                'bar-empty': audienceTotalVotes === 0,
              }"
            />
          </div>
          <div class="opt-stat">
            <span class="stat-count">{{ audienceVotes[i] || 0 }}</span>
            <span class="stat-pct">{{ audiencePct(i) }}%</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="!audienceActive && audienceConnected" class="empty-state">No active poll on this slide</div>
  </div>
</template>
...
import type { ActivePollMap, Poll, PollOption, RawQuestion, ServerMessage } from "../types";
...
const slidePolls = computed(() => polls.value.filter((p) => Number(p.slideIndex) === slideIndex));
const hasPolls = computed(() => slidePolls.value.length > 0);

const activeId = computed(() => {
  if (!hasPolls.value) return null;
  const mapped = activeBySlide.value[String(slideIndex)];
  if (mapped) return mapped;
  const first = pollIds[0];
...
function selectPoll(id: string) {
  activeBySlide.value[String(slideIndex)] = id;
  sendToServer({ type: "poll_selected", slideIndex, pollId: id });
}
function nextPoll() {
...
function resetPoll() {
  ...
  const firstId = pollIds[0] ?? slidePolls.value[0]?.id;
...
const audienceActive = computed<Poll | null>(() => {
  const ps = localPolls.value.filter((p) => Number(p.slideIndex) === slideIndex);
  if (!ps.length) return null;
  const aid = localActiveBySlide.value[String(slideIndex)];
  if (aid) {
...
const audienceQuestion = computed(() => audienceActive.value?.question ?? "Waiting…");
const audienceVotes = computed(() => audienceActive.value?.votes ?? []);
const audienceTotalVotes = computed(() => audienceVotes.value.reduce((a, b) => a + b, 0));
const showReveal = computed(
  () => audienceActive.value?.type === "quiz" && audienceActive.value?.revealed === true,
);

const audienceWcWords = computed(() => {
  if (!audienceActive.value) return [];
  const wc = audienceActive.value.wordCounts ?? {};
  return Object.entries(wc)
    .map(([word, count]) => ({ word, count }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 30);
});
const audienceWcMax = computed(() => Math.max(...audienceWcWords.value.map((w) => w.count), 1));

function audiencePct(i: number): number {
  return audienceTotalVotes.value
    ? Math.round(((audienceVotes.value[i] ?? 0) / audienceTotalVotes.value) * 100)
    : 0;
}
function audienceIsLead(i: number): boolean {
  if (!audienceVotes.value.length) return false;
  const mx = Math.max(...audienceVotes.value);
  return mx > 0 && audienceVotes.value[i] === mx;
}
...
onMounted(() => {
  if (!isPresenter.value) {
    audienceWs = createAudienceWs(handleAudienceMsg);
  }
});
onUnmounted(() => {
  if (audienceWs) {
...
.poll-presenter-root { max-width: 900px; }
.poll-audience-root  { max-width: 700px; }
